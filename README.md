...
## Definition
Ponce (pronuntiated *[ 'poN θe ]* ) is an IDA Pro plugin that provides users the ability to perform taint analysis and symbolic execution over binaries in an easy and intuitive fashion. The plugin is entirely written in C/C++.
...
## Use cases
- Esploit development: Ponce can help you creating an exploit since you can easily see what parts of memory and registers you control and posible addresses you can use as ROP gadgets.
...
## Installation
Ponce works with both x86 and x64 binaries. Installing the plugin is as simple as copying [Ponce.plw]() and [Ponce.p64]() to the [IDA Path]\plugins folder. The plugin will automatically run asking you for the initial configuration the fist time you use it and the configuration will be saved to a configuration file so you won't have to worries about the config windows again.
...
The static version of ```z3.lib``` is ~ 1.1Gb and the linking time is considerable. That's the main reason why we have a building version that doesn't use this library but dynamically links against z3.
...
- Francisco Oca (@francisco_oca) francisco.oca.gonzalez@gmail.com
