...
struct ah_taint_symbolize_memory_t : public action_handler_t
{
    ...
    virtual int idaapi activate(action_activation_ctx_t* ctx)
    {
        ...
        for (unsigned int i = 0; i < selection_length; i++) {
          needConcreteMemoryValue_cb(
              tritonCtx, triton::arch::MemoryAccess(selection_starts + i, 1));
        }

        if (cmdOptions.use_tainting_engine) {
            for (unsigned int i = 0; i < selection_length; i++) {
            auto taintVar = tritonCtx.taintMemory(
                triton::arch::MemoryAccess(selection_starts + i, 1));
                if (taintVar) {
            ...
            for (unsigned int i = 0; i < selection_length; i++) {
            auto symVar = tritonCtx.symbolizeMemory(
                triton::arch::MemoryAccess(selection_starts + i, 1));
